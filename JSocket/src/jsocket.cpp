...
namespace Tools
{
    ...
    CSocket::CSocket(SocketType type, NetworkLayerProtocol protocol, int timeOutSec, int timeOutUSec) throw(CSocketException): m_sockDesc(-1)
    {
    ...
    int CSocket::Recv(void *buffer, int bufferLen) throw(CSocketException)
    {
        ...
        setsockopt(m_sockDesc, SOL_SOCKET, SO_RCVTIMEO, (char *)&tv, sizeof(struct timeval));
